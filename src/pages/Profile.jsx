...
import { getRascunhosByUsuario } from '../services/capituloService';
...
const Profile = () => {
  ...
  const loadRascunhos = async () => {
    try {
      ...
      const data = await getRascunhosByUsuario(user.id);
      setRascunhos(data || []);
    } catch (error) {
        ...
        {user.tipoUsuario?.id === 1 && (
          <div className="drafts-section">
            <div className="drafts-header">
              ...
              <span className="drafts-count">{rascunhos.length} rascunho{rascunhos.length !== 1 ? 's' : ''}</span>
            </div>
            ...
            {loadingRascunhos ? (
              ...
              <div className="empty-drafts">
                ...
                <span>Crie rascunhos ao editar capítulos para salvar seu progresso</span>
              </div>
            ) : (
              <div className="drafts-grid">
                {rascunhos.map((rascunho) => (
                  <div key={rascunho.id} className="draft-card">
                    <div className="draft-header">
                      ...
                      <div className="draft-badge">Rascunho</div>
                    </div>
                    <div className="draft-content">
                      <h3 className="draft-name">{rascunho.nome}</h3>
                      <div className="draft-info">
                        <span className="draft-book">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path>
                            <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path>
                          </svg>
                          {rascunho.livro_titulo}
                        </span>
                        <span className="draft-chapter">
                          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                            <polyline points="14 2 14 8 20 8"></polyline>
                          </svg>
                          {rascunho.capitulo_titulo}
                        </span>
                      </div>
                    </div>
                    <div className="draft-actions">
                      <button 
                        ...
                        onClick={() => navigate(`/livro/${rascunho.id_livro}/capitulo/${rascunho.id}/secoes`)}
                      ...
                      <button 
                        className="btn-draft-view"
                        onClick={() => navigate(`/livro/${rascunho.id_livro}/capitulo/${rascunho.id_capitulo_original}/secoes`)}
                        title="Ver capítulo original"
                      >
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
